...
public:
    ...
    void Collect( VkCommandBuffer cmdbuf )
    {
        ...
        if( vkGetQueryPoolResults( m_device, m_query, wrappedTail, cnt, sizeof( int64_t ) * m_queryCount, m_res, sizeof( int64_t ), VK_QUERY_RESULT_64_BIT ) == VK_NOT_READY )
        {
            m_oldCnt = cnt;
            return;
        }
        ...
        vkCmdResetQueryPool( cmdbuf, m_query, wrappedTail, cnt );
